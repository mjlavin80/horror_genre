...
from ratio_functions import oed_gl_ratio
    ...
    for count, dataset in enumerate(all_data):
        if len(dataset) == 0:
            ...
            if original_length >= 5000:
                ...
                gl_ratio_no_set, matched_no_set, passed_no_set, neo = oed_gl_ratio(expanded, oed_dictionary)

                #to analyze as set, run same function on as_set
                as_set = [f[0] for f in no_fw]
                gl_ratio_set, matched_set, passed_set, neo_set = oed_gl_ratio(as_set, oed_dictionary)

                #oed_matches
                oed_matches = {"matched_set":matched_set, "matched_no_set": matched_no_set, "passed_set": passed_set, "passed_no_set": passed_no_set, "neo_set": neo_set, "neo_no_set": neo}
                ...
                gl_resample_set =[]
                for i in resamples:
                    results = oed_gl_ratio(i, oed_dictionary)
                    gl_resample_no_set.append(results[0])
                    set_results = oed_gl_ratio(set(i), oed_dictionary)
                ...
                data_tuple = (0, 0, year, datakey[count], original_length, tt_ratio, json.dumps(oed_matches), gl_ratio_set, gl_ratio_no_set, json.dumps(gl_resample_set), json.dumps(gl_resample_no_set), json.dumps(resample_tt_ratios))
            else:
        ...
        conn = sqlite3.connect('gl_ratios_oed.db')
        c = conn.cursor()
        create = """CREATE TABLE IF NOT EXISTS results (id INTEGER PRIMARY KEY,
                     ...
                     tt_ratio real, oed_matches blob, gl_ratio_set real, gl_ratio_no_set real,
        ...
        tt_ratio, oed_matches, gl_ratio_set, gl_ratio_no_set, gl_resample_set, gl_resample_no_set,
        resample_tt_ratios) VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""
        c.execute(insert, data_tuple)
        conn.commit()
        #data_tuple = (0, 0, year, datakey[count], original_length, tt_ratio, oed_matches, gl_ratio_set, gl_ratio_no_set, gl_resample_set, gl_resample_no_set, resample_tt_ratios )
